...
<html>
	<head>
		<meta charset="utf-8">
		<title>Geolocator Results</title>
	...
	<body>
		...
		<?php
			...
			$geocoder = new \OpenCage\Geocoder\Geocoder('8a8e4dc9d2c845a1a1a05cc3cc9fd1ff');

			$lat_long = '43.831,4.360';
			$lat_long = 'Berlin';
			...
			echo '<h1>Geolocator results for: ' . $_POST['q'] . '</h1>';
			echo '<h2>Source: ' . $searchResult['results'][0]['source'] . '</h2>';
			// echo $entry['formatted'];
		...
		<?php
			...
			foreach ($searchResult['results'] as $entry) {

				echo '<tr>';
					echo '<td>' . $entry['geometry']['lat'] . '</td>';
					echo '<td>' . $entry['geometry']['lng'] . '</td>';
					echo '<td>' . $entry['countryCode'] . '</td>';
					echo '<td>' . $entry['timezone'] . '</td>';
				echo '</tr>';
			}
